...
export default function LoginPage() {
  ...
  useEffect(() => {
    async function fetchOrganizations() {
      try {
        ...
        if (response.ok) {
          setOrganizations((data || []).filter((org: Organization) => !org.disabled));
        }
      } catch (err) {
        // Silently fail — user can still login without org selection
      }
    }
    fetchOrganizations();
  }, []);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (
        payload.session?.role === "super_admin" ||
        payload.session?.role === "admin" ||
        payload.session?.role === "manager"
      ) {
  ...
  // Get color for org avatar
  function getOrgColor(index: number): string {
    const colors = ["bg-blue-600", "bg-amber-500", "bg-emerald-600", "bg-orange-500", "bg-red-500", "bg-purple-600", "bg-pink-500"];
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        {registered && (
          <p className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 font-medium text-center">
            Account created successfully! Log in to begin.
          </p>
        )}
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Brand (optional for customers)</label>
            <div className="relative" ref={dropdownRef}>
              <button
                type="button"
                onClick={() => setOrgDropdownOpen(!orgDropdownOpen)}
                className="flex h-11 w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 text-sm transition-colors hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <div className="flex items-center gap-2">
                  ...
                  {selectedOrg ? (
                    ...
                    <span className="text-slate-400">Search brand or login by email...</span>
                  )}
                </div>
                  ...
                  <div className="p-2 border-b border-slate-100">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full h-9 pl-9 pr-3 text-sm rounded-lg border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        autoFocus
                      />
                    </div>
                  </div>
                    ...
                    {filteredOrgs.length > 0 ? (
                      filteredOrgs.map((org, index) => (
                        <button
                          ...
                          className={`flex w-full items-center justify-between px-3 py-2.5 text-sm hover:bg-slate-50 transition-colors ${
          ...
          {/* Error */}
          {error && (
            <p className="rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-600 font-medium">
              {error}
            </p>
          )}
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center">
            <span className="bg-slate-50 px-3 text-sm text-slate-400">or</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 pt-4 text-xs text-slate-400">
          ...
          <div>
            <span className="font-semibold text-slate-500">Secure. Private. Reliable.</span>
            <br />
